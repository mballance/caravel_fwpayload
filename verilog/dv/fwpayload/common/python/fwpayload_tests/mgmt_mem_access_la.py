...
#    await la_utils.set_dut_clock_control(False)
    ...
    for i in range(16):
        data = await la_utils.read(0x30000000 + 4*i)
        if wr_data[i] == data:
    ...
    for i in range(32):
        ...
        await la_utils.write(0x30000000 + 2*i, data, 
                         0x3 if (i%2) == 0 else 0xC)
        print("wr_data[" + str(i) + "] = " + hex(wr_data[i]))
        
    for i in range(32):
        data = await la_utils.read(0x30000000 + 2*i)
        if (i%2) != 0:
            data >>= 16
        data &= 0xFFFF
        if wr_data[i] == data:
    ...
    for i in range(64):
        ...
        await la_utils.write(0x30000000 + i, data, (1 << (i%4)))
        print("wr_data[" + str(i) + "] = " + hex(wr_data[i]))
        
    for i in range(64):
        data = await la_utils.read(0x30000000 + i)
        data >>= (8*(i%4))
        data &= 0xFF
        if wr_data[i] == data:
    ...
    for i in range(64):
        await la_utils.write(0x32000000 + i, data, 0xF)
 
    for i in range(64):
        await la_utils.read(0x32000000 + i)
